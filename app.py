...
def answer_question(student_id, question, state):
    ...
    if any(k in ql for k in ["offered", "offer", "available"]) and "semester" in ql:
        ...
        if m:
            target = "S" + m.group(1)
            x = offerings[
                offerings["semester"].astype(str).str.upper() == target
            ]
            if x.empty:
                return f"**{title} ({code})** is not listed in the provided semester-offering data for {target}.", state, retrieved
            structures = ", ".join(sorted(x["academic_structure"].dropna().astype(str).unique()))
            return (
                f"**{title} ({code})** is listed for **{target}** in the provided offering data.\n\n"
...
st.markdown("---")
st.markdown(
    '<div class="small-note">🔒 Grounding rule: the advisor uses the provided university database and synthetic student data. '

...
if LOGO:
    ...
    logo_html = f'<img class="vu-logo" src="data:{mime};base64,{b64}">'
else:
    logo_html = '<div class="vu-logo" style="background:#123b86;display:flex;align-items:center;justify-content:center;color:white;font-size:24px;">V</div>'
...
<div class="vu-header">
  <div class="vu-brand">
    {logo_html}
    <div>
      <div class="vu-name">VIDYASHILP UNIVERSITY</div>
      <div class="vu-sub">AI Academic Advisor</div>
    </div>
  </div>
  <div class="vu-status"><span class="dot"></span> Advisor online</div>
</div>
""", unsafe_allow_html=True)
...
<div class="hero">
  ...
  <p>Ask naturally about courses, prerequisites, credits, offerings, degree requirements, or your academic record.</p>
  <div class="badges">
    <span class="badge">Grounded in university data</span>
    <span class="badge">Course-name friendly</span>
    <span class="badge">No guessing</span>
  </div>
</div>
""", unsafe_allow_html=True)
...
<div class="robot-wrap">
  <div class="robot"><span class="eye1"></span><span class="eye2"></span></div>
  <div>
    <div class="robot-name">AIRA — Academic Intelligence & Registration Assistant</div>
    <div class="robot-msg">Hi! Ask me a question. I’ll check the university data before answering.</div>
  </div>
</div>
""", unsafe_allow_html=True)
...
if "advisor_state" not in st.session_state: st.session_state.advisor_state={}

for msg in st.session_state.messages:
    with st.chat_message(msg["role"]):
        st.markdown(msg["content"])

st.markdown('<div class="chat-card"><div class="chat-title">Ask AIRA</div><div class="chat-sub">Type your question below — you don’t need to know the course code.</div></div>', unsafe_allow_html=True)

q1,q2,q3=st.columns(3)
examples=[
    "What is the prerequisite for Leadership and Teamwork Skills?",
    "How many credits is Communication Skills?",
    "What are the graduation requirements for Struct_2024?"
]
for col,ex in zip([q1,q2,q3],examples):
    if col.button(ex,use_container_width=True):
        st.session_state.pending_question=ex

question=st.chat_input("Type your academic question…")
...
if question:
    ...
    answer,new_state,retrieved=answer_question(selected_student,question,st.session_state.advisor_state)
    elapsed=(time.perf_counter()-start)*1000
    st.session_state.advisor_state=new_state
    st.session_state.messages.append({"role":"assistant","content":answer})
    with st.chat_message("assistant"):
        st.markdown(answer)
        st.caption(f"Checked against university data • {elapsed:.1f} ms")
        if retrieved is not None and not retrieved.empty:
            with st.expander("View evidence"):
                ...
                if "retrieval_score" in ev: ev["retrieval_score"]=ev["retrieval_score"].round(3)
                st.dataframe(ev,use_container_width=True,hide_index=True)

# ---------- MINIMAL FOOTER ----------
st.markdown("---")
st.caption("Vidyashilp University • AI Academic Advisor • Answers are limited to the provided university data.")
